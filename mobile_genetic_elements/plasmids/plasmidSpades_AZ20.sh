...
sed -n "${SLURM_ARRAY_TASK_ID}p" /home/callaban/Projects/Adaptation/AZ20_list_leA.txt | while read smp r1 r2 x; do 
	...
	out_dir=/home/callaban/scratch2/plasmidspades_AZ20/${nme}
	if [ ! -s $out_dir/scaffolds.fasta ]; then
		plasmidspades.py -t 16 -1 ${r1} -2 ${r2} --tmp-dir /home/callaban/scratch2/tmp/ --cov-cutoff 50 -m 20 -o ${out_dir}
		bioawk -c fastx 'length($seq) > 3000 {print ">"$name"\n"$seq"\n"}' ${out_dir}/scaffolds.fasta > ${out_dir}/large_plasmids.fasta
	fi
done

#generally <10 min, 8GB